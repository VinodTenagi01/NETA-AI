...
class FieldReport(Base):
    ...
    escalation = relationship("Escalation", back_populates="field_report", uselist=False)
    ...
    __table_args__ = (
        ...
        Index("idx_field_reports_reported_by", "reported_by", "created_at"),
    )
...
class Escalation(Base):
    ...
    field_report_id = Column(UUID(as_uuid=True), ForeignKey("field_reports.id"), nullable=True, unique=True)
    ...
    field_report = relationship("FieldReport", back_populates="escalation", foreign_keys=[field_report_id])
    ...
    __table_args__ = (
        ...
        Index("idx_escalations_status_deadline", "status", "sla_deadline"),
        Index("idx_escalations_assigned_to", "assigned_to", "status"),
    )
...
class Voter(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    booth_id = Column(UUID(as_uuid=True), ForeignKey("booths.id"), nullable=False)
    voter_id = Column(String(50), unique=True)          # EPIC number e.g. SWO5869530
    serial_number = Column(Integer)                      # position within part
    full_name = Column(String(255), nullable=False)
    father_name = Column(String(255))                    # father or husband name from roll
    ...
    __table_args__ = (
        ...
        Index("idx_voters_serial", "booth_id", "serial_number"),
    )
...
class WorkerAttendance(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    booth_id = Column(UUID(as_uuid=True), ForeignKey("booths.id"), nullable=False)
    zone_id = Column(UUID(as_uuid=True), ForeignKey("campaign_zones.id"), nullable=False)
    ...
    zone = relationship("CampaignZone", foreign_keys=[zone_id])

    __table_args__ = (
        Index("idx_worker_attendance_user_checkin", "user_id", "checked_in_at"),
        Index("idx_worker_attendance_zone_checkin", "zone_id", "checked_in_at"),
        Index("idx_worker_attendance_checkout", "checked_out_at"),
    )
...
class MoodSnapshot(Base):
    ...
    avg_sentiment_score = Column(Numeric(4, 3), nullable=False)
    positive_pct = Column(Numeric(5, 2), default=0.0)
    neutral_pct = Column(Numeric(5, 2), default=0.0)
    negative_pct = Column(Numeric(5, 2), default=0.0)
    mixed_pct = Column(Numeric(5, 2), default=0.0)
    ...
    __table_args__ = (
        UniqueConstraint("zone_id", "snapshot_date", name="uq_mood_zone_date"),
        Index("idx_mood_snapshots_zone_date", "zone_id", "snapshot_date"),
    )
