...
class GeoJSONMappingService:
    ...
    async def get_booth_popup(
        ...
        # Real voter stats from voters table (falls back to booth metadata if no rows yet)
        voter_stats_result = await db.execute(
            select(
                func.count().label("total"),
                func.sum(case((Voter.is_contacted == True, 1), else_=0)).label("contacted"),
            ).where(Voter.booth_id == booth_id)
        )
        voter_stats = voter_stats_result.first()
        db_voter_count = voter_stats.total if voter_stats and voter_stats.total else 0
        contact_count = int(voter_stats.contacted or 0) if voter_stats else 0
        total_v = db_voter_count if db_voter_count > 0 else (booth.total_voters or 0)
        ...
        contact_pct = round(contact_count / total_v * 100, 1) if total_v > 0 else float(booth.contact_rate or 0)
